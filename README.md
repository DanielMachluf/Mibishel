...
Dishshare is a full-stack recipe collection app built around a simple idea: paste a recipe link from TikTok, Instagram, or Facebook, let automation extract the useful cooking data, and keep the result in a clean private dashboard.
...
- The backend package name still says `northwind-rest-api`; the frontend package name still says `northwind`. The app itself is built as Dishshare.
...
- Rename package metadata from the old Northwind names to Dishshare.
